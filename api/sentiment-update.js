...
const FOOTBALL_KEYWORDS = [
  // Football terms
  'soccer', 'fußball', 'fussball', 'football', 'futbol', 'fútbol', 'futebol', 'calcio', 'voetbal',
  // FIFA specific
  ...
  'nationalmannschaft', 'national team', 'équipe nationale', 'seleção',
  
  // ==================== CONTINENTAL FEDERATIONS ====================
  'uefa', 'conmebol', 'concacaf', 'afc', 'caf', 'ofc',
  
  // ==================== EUROPEAN FEDERATIONS (UEFA) ====================
  'dfb', 'öfb', 'oefb', 'sfv', 'asf', 'knvb', 'fff', 'figc', 'rfef', 'fpf',
  ...
  'messi', 'mbappé', 'mbappe', 'haaland', 'bellingham', 'vinicius', 'neymar',
  'müller', 'muller', 'kane', 'ronaldo', 'salah', 'de bruyne', 'kroos',
  'gündogan', 'gundogan', 'sané', 'sane', 'havertz', 'musiala', 'wirtz',
  
  // ==================== FOOTBALL-SPECIFIC TERMS ====================
  'striker', 'goalkeeper', 'midfielder', 'defender', 'stürmer', 'torwart',
...
const GOOGLE_NEWS_FEEDS = [
  ...
  { url: 'https://news.google.com/rss/search?q="Fußball+WM+2026"&hl=de&gl=DE', lang: 'de', region: 'DE' },
  { url: 'https://news.google.com/rss/search?q="FIFA+WM+2026"&hl=de&gl=DE', lang: 'de', region: 'DE' },
  { url: 'https://news.google.com/rss/search?q="Weltmeisterschaft+2026"+Fußball&hl=de&gl=DE', lang: 'de', region: 'DE' },
  { url: 'https://news.google.com/rss/search?q="Mundial+2026"+fútbol&hl=es&gl=ES', lang: 'es', region: 'ES' },
...
async function fetchNewsAPI() {
  ...
  try {
    const response = await fetch(
      `https://newsapi.org/v2/everything?q=("FIFA World Cup 2026" OR "Fußball WM 2026" OR "soccer world cup 2026")&language=en&sortBy=publishedAt&pageSize=100`,
...
// ==================== NEW: MASTODON ====================
// ✅ Vollständig offen - Dezentrale Instanzen
...
async function fetchBluesky() {
  ...
  console.log(`🦋 Bluesky: ${allPosts.length} posts fetched`);
  return allPosts;
}

// ==================== NEW: THREADS (Meta) ====================
// ⚠️ Eingeschränkt - Über öffentliche Profile/Web scraping
async function fetchThreads() {
  // Threads hat keine vollständige öffentliche API
  // Wir können die öffentliche Graph API für Business-Accounts nutzen
  // oder populäre Fußball-Hashtags über alternative Methoden abrufen
  
  const accessToken = process.env.THREADS_ACCESS_TOKEN;
  
  // Wenn kein Token verfügbar, versuche öffentliche Daten
  if (!accessToken) {
    console.log('🧵 Threads: No access token, skipping (limited API)');
    return [];
  }
  
  try {
    // Meta's Threads API (für authentifizierte Requests)
    ...
    console.log(`🧵 Threads: ${posts.length} posts fetched`);
    return posts;
...
export default async function handler(req, res) {
  ...
  try {
    ...
    console.log(`⚽ After football filter: ${footballArticles.length}`);
    ...
    console.log(`📊 Score: ${score}, Label: ${label.en}, Trend: ${trend}`);
    console.log(`📰 News: ${byType.news.score} (${byType.news.count}) | 💬 Social: ${byType.social.score} (${byType.social.count})`);
